...
def get_installed_chrome_version(binary_path):
    """
    Check the version of the Chrome binary at the given path.
    """
    if not binary_path or not os.path.exists(binary_path):
        return None
    
    try:
        # Run the binary with --version to get the version string
        ...
        # Extract version pattern like 120.0.6099.109
        match = re.search(r"(\d+\.\d+\.\d+\.\d+)", version_output)
        if match:
            return match.group(1)
    except Exception as e:
        logger.warning(f"Could not detect version for binary {binary_path}: {e}")
